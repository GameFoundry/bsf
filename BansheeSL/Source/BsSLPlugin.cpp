...
namespace BansheeEngine
{
	...
	Vector<CodeBlock> parseCodeBlocks(String& data)
	{
		...
		UINT32 offset = 0;
		while (std::regex_search(data.cbegin() + offset, data.cend(), matches, pattern))
		{
			...
			for (UINT32 i = offset; i < (UINT32)data.length(); i++)
			{
				if (data[i] == '{')
					ummatchedBrackets++;
			...
			data.erase(data.cbegin() + offset, data.cbegin() + offset + (UINT32)newBlock.code.size());
	...
	StringID parseRenderer(const String& name)
	{
		if (name == "Any")
			return RendererAny;
	...
	void parseLanguage(const String& name, StringID& renderAPI, String& language)
	{
	...
	GpuParamBlockUsage parseBlockUsage(BufferUsageValue usage)
	{
	...
	UINT32 parseFilterMode(FilterValue filter)
	{
		switch (filter)
		{
		case FV_Point:
	...
	CompareFunction parseCompFunc(CompFuncValue compFunc)
	{
		switch (compFunc)
		{
		case CFV_Pass:
	...
	TextureAddressingMode parseAddrMode(AddrModeValue addrMode)
	{
		switch (addrMode)
		{
		case AMV_Wrap:
	...
	BlendFactor parseBlendFactor(OpValue factor)
	{
		switch (factor)
		{
		case OV_One:
	...
	BlendOperation parseBlendOp(BlendOpValue op)
	{
		switch (op)
		{
		case BOV_Add:
	...
	String removeQuotes(const char* input)
	{
	...
	void parseParamType(ParamType type, bool& isObjType, UINT32& typeId)
	{
		struct ParamData
		{
			UINT32 type;
	...
	StencilOperation parseStencilOp(OpValue op)
	{
		switch (op)
		{
		case OV_Keep:
	...
	CullingMode parseCullMode(CullModeValue cm)
	{
		switch (cm)
		{
		case CMV_None:
	...
	PolygonMode parseFillMode(FillModeValue fm)
	{
		if (fm == FMV_Wire)
			return PM_WIREFRAME;
	...
	void parseStencilFront(DEPTH_STENCIL_STATE_DESC& desc, ASTFXNode* stencilOpNode)
	{
	...
	void parseStencilBack(DEPTH_STENCIL_STATE_DESC& desc, ASTFXNode* stencilOpNode)
	{
		if (stencilOpNode == nullptr || stencilOpNode->type != NT_StencilOp)
			return;
	...
	void parseAddrMode(SAMPLER_STATE_DESC& desc, ASTFXNode* addrModeNode)
	{
	...
	void parseColorBlendDef(RENDER_TARGET_BLEND_STATE_DESC& desc, ASTFXNode* blendDefNode)
	{
	...
	void parseAlphaBlendDef(RENDER_TARGET_BLEND_STATE_DESC& desc, ASTFXNode* blendDefNode)
	{
		if (blendDefNode == nullptr || blendDefNode->type != NT_BlendDef)
			return;
	...
	void parseRenderTargetBlendState(BLEND_STATE_DESC& desc, ASTFXNode* targetNode)
	{
	...
	HBlendState parseBlendState(ASTFXNode* passNode)
	{
	...
	HRasterizerState parseRasterizerState(ASTFXNode* passNode)
	{
		if (passNode == nullptr || passNode->type != NT_Pass)
	...
	HDepthStencilState parseDepthStencilState(ASTFXNode* passNode)
	{
		if (passNode == nullptr || passNode->type != NT_Pass)
	...
	HSamplerState parseSamplerState(ASTFXNode* samplerStateNode)
	{
	...
	GpuProgramProfile getProfile(const StringID& renderAPI, GpuProgramType type)
	{
	...
	PassPtr parsePass(ASTFXNode* passNode, const Vector<CodeBlock>& codeBlocks, const Vector<String>& includes, const StringID& renderAPI, const String& language)
	{
		if (passNode == nullptr || passNode->type != NT_Pass)
			return nullptr;
	...
	TechniquePtr parseTechnique(ASTFXNode* techniqueNode, const Vector<CodeBlock>& codeBlocks)
	{
	...
	void parseParameters(SHADER_DESC& desc, ASTFXNode* parametersNode)
	{
	...
	void parseBlocks(SHADER_DESC& desc, ASTFXNode* blocksNode)
	{
	...
	HShader createShader(const String& name, ASTFXNode* rootNode, const Vector<CodeBlock>& codeBlocks)
	{
		...
		return Shader::create(name, shaderDesc, techniques);
	}
	...
	extern "C" BS_SL_EXPORT void* loadPlugin()
	{
		...
		Vector<CodeBlock> codeBlocks = parseCodeBlocks(contents);
		...
		{
			HShader shader = createShader("Shader", parseState->rootNode, codeBlocks);

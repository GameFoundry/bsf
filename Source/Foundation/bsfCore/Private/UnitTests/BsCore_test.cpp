...
class CoreTestSuite : public bs::Test {};
...
TEST(CoreTestSuite, testAnimCurveIntegration)
{
...

	{
		...
		ASSERT_NEAR(
			curveConstant.evaluateIntegrated(0.0f, cache), 0.0f, EPSILON);
		ASSERT_NEAR(
			curveConstant.evaluateIntegrated(0.5f, cache), 0.5f, EPSILON);
		ASSERT_NEAR(
			curveConstant.evaluateIntegrated(1.0f, cache), 1.0f, EPSILON);
	}

	{
		TCurveIntegrationCache<float> cache;
		ASSERT_NEAR(
			curveLinear.evaluateIntegrated(0.0f, cache), 0.0f, EPSILON);
		ASSERT_NEAR(
			curveLinear.evaluateIntegrated(0.5f, cache), 0.125f, EPSILON);
		ASSERT_NEAR(
			curveLinear.evaluateIntegrated(1.0f, cache), 0.5f, EPSILON);
	}

	{
		...
		for(auto time : times)
		{
			ASSERT_NEAR(curveConstant.evaluateIntegratedDouble(time, cache), 
...

	{
		...
		for(auto time : times)
		{
			ASSERT_NEAR(curveAcceleration.evaluateIntegrated(time, cache), 
					evalVelocity(-9.81f, time), EPSILON);
		}

		std::pair<float, float> range = curveAcceleration.calculateRangeIntegrated(cache);
		ASSERT_NEAR(range.first, -98.1f, EPSILON);
		ASSERT_NEAR(range.second, 0.0f, EPSILON);
	}

	{
		...
		for(auto time : times)
		{
			ASSERT_NEAR(curveAcceleration.evaluateIntegratedDouble(time, cache), 
					evalPosition(-9.81f, 0.0f, time), EPSILON);
		}

		std::pair<float, float> range = curveAcceleration.calculateRangeIntegratedDouble(cache);
		ASSERT_NEAR(range.first, -490.5f, EPSILON);
		ASSERT_NEAR(range.second, 0.0f, EPSILON);
	}
}

TEST(CoreTestSuite, testLookupTable)
{
	static constexpr float EPSILON = 0.001f;
	...
	for(UINT32 i = 0; i < 10; i++)
	{
		...
		for(UINT32 j = 0; j < 3; j++)
			ASSERT_NEAR(valueLookup[j], valueCurve[j], EPSILON);
	}
}

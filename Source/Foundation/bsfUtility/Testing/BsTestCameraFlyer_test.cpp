...
namespace bs
{
	...
	void CameraFlyer::update()
	{
		...
		if (camRotating)
		{
			...
			Degree mYaw = Degree(
				gVirtualInput().getAxisValue(mHorizontalAxis) * ROTATION_SPEED);
			Degree mPitch = Degree(
				gVirtualInput().getAxisValue(mVerticalAxis) * ROTATION_SPEED);
		...
		if (direction.squaredLength() != 0)
		{
			...
			mCurrentSpeed = Math::clamp(
				mCurrentSpeed + ACCELERATION * frameDelta, START_SPEED, TOP_SPEED);

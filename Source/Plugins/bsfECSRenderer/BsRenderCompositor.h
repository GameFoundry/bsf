...
namespace ct
{
	...
	struct RenderCompositorNodeInputs
	{
		RenderCompositorNodeInputs(const RendererViewGroup& viewGroup, const RendererView& view, const SceneInfo& scene,
			const RenderECSOptions& options, const FrameInfo& frameInfo, RenderECSFeatureSet featureSet)
		...
		const RenderECSOptions& options;
		const FrameInfo& frameInfo;
		const RenderECSFeatureSet featureSet;

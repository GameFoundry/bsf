...
namespace bs { namespace ct
{
	MaterialSamplerOverrides* SamplerOverrideUtility::generateSamplerOverrides(const SPtr<Shader>& shader,
		...
		const SPtr<RenderECSOptions>& options)
	{
	...
	bool SamplerOverrideUtility::checkNeedsOverride(const SPtr<SamplerState>& samplerState, const SPtr<RenderECSOptions>& options)
	{
		const SamplerProperties& props = samplerState->getProperties();

		switch (options->filtering)
		{
		case RenderECSFiltering::Bilinear:
		{
			...
			break;
		case RenderECSFiltering::Trilinear:
		{
			if (props.getTextureFiltering(FT_MIN) != FO_LINEAR)
				return true;
			...
			break;
		case RenderECSFiltering::Anisotropic:
		{
	...
	SPtr<SamplerState> SamplerOverrideUtility::generateSamplerOverride(const SPtr<SamplerState>& samplerState, const SPtr<RenderECSOptions>& options)
	{
		const SamplerProperties& props = samplerState->getProperties();
